...
#log文件位置
filename = re.sub(r'[^0-9]', '_', str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), 0)  #log 文件名
save_file_path = 'D:/00-PC-TOOL/c201-s/'+filename+'.txt'  # log目录（需手动修改）
...
#升级的sw
Upgrade_SW_List = ["00.01.24.bin","00.01.25.bin","00.01.26.bin"]  #待测版本（需手动修改）
Upgrade_SW_Dir = r"D:/00-PC-TOOL/c201-s/"  #待测版本的目录文件夹（需手动修改）
