...
def Upgrade(sw,delay_time):										  # 定义升级函数：sw 是要升级的文件，delay_time是升级版本给定的时间。
	...
	if (tab_1.Screen.WaitForString('not found',2)):
		crt.Dialog.MessageBox(str(sw) + "  该版本不存在","session",32|3)
		time.sleep(1)
		crt.Dialog.MessageBox(check_version+ '  '+'current_sw=  ',+ current_sw,"session",32|3)      #### 可以用来终止程序
		time.sleep(100)
		sys.exit(0)
...
def check_Version(sw_version):
	...
	if version_result == 1:											#检测版本是否存在，如果不存在，可能烧成砖了。如果存在，则提取版本号
		current_sw = tab_2.Screen.ReadString('CPU2').strip()
		current_sw = current_sw[:8]
		
		#current_sw = str(tab_2.Screen.ReadString('CPU2')).split()
		#current_sw0 = current_sw[0]
		#current_sw1 = current_sw[3]
		#crt.Dialog.MessageBox(current_sw,"session",32|3)
		time.sleep(2)
	...
	if (current_sw == check_version):								#版本检测，查看升级是否成功
		...
		#crt.Dialog.MessageBox(check_version,"session",32|3)
		filep.write(str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))+'  '+str(current_sw)+str(check_version)+'   PASS'+'\r\n')
			
	else:
		filep = open(file_path, 'a+')
		crt.Dialog.MessageBox(check_version,"session",32|3)
		filep.write(str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))+'  '+'check_version=  '+ str(check_version)+ '  '+'current_sw=  '+ str(current_sw)+'   Fail'+'\r\n')
		crt.Dialog.MessageBox('check_version=  '+ check_version+ '  '+'current_sw=  ',+ current_sw,"session",32|3)      #### 可以用来终止程序
		time.sleep(100)
		return
...
#############升级8M (0~8) app，再升级00.01.25，再随机随机一个full_image,最后升级4M BootLoader#################################################
		...
		for i in range(random.randint(0,8)):  								#升级随机的<8个 8M_app.bin
			...
			Upgrade(upgrade_SW_8M[a],check_version_8M[a],100)

			
		Upgrade(upgrade_SW_8M[-1],check_version_8M[-1],100)					#升级到最新的那个版本

		
		Upgrade(upgrade_FULL_SW_8M[a],check_version_8M[a],150) 				#升级随机一个8M_full_image.bin
...
#############升级4M (0~8) app，再升级00.01.16，最后升级8M BootLoader##########################################################################
		...
		for i in range(random.randint(0,8)):  								#升级随机的<8个 4M_app.bin
			...
			Upgrade(upgrade_SW[a],check_version[a],100)

		Upgrade(upgrade_SW[0],check_version[0],100)							#升级到最旧的那个版本
